...
const OurGallery = () => {
    ...
    const fetchMemories = async () => {
        try {
            const res = await axios.get('http://localhost:5000/api/memories/all');
    ...
    const confirmDelete = async () => {
        try {
            await axios.delete(`http://localhost:5000/api/memories/${deleteId}`);
            setMemories(prev => prev.filter(m => m._id !== deleteId));
            setDeleteId(null);
        } catch (err) { alert('Could not remove this memory.'); }
    };
            ...
            {deleteId && (
                <div className={styles.confirmOverlay}>
                    <div className={styles.confirmBox}>
                        <h3>Remove Memory?</h3>
                        <p>This moment will be gone forever.<br />Are you sure?</p>
